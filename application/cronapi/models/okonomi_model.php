<?php
  class Okonomi_model extends CI_Model {
    ...
    function faktura($ID) {
      $rfakturaer = $this->db->query("SELECT FakturaID,Referanse,DatoFakturadato,Notater,PersonAnsvarligID,OrganisasjonID,(SELECT SUM(Pris*Antall) FROM FakturaLinjer WHERE (FakturaLinjer.FakturaID=f.FakturaID)) AS FakturaSum,StatusID FROM Fakturaer f WHERE (FakturaID=".$ID.") LIMIT 1");
    ...
    function lagrefaktura($ID = null, $faktura) {
      ...
      if ($ID == null) {
        ...
        if ($faktura['StatusID'] == 2) {
          $gfaktura = $this->faktura($ID);
          $this->lagreinntekt(null,array('PersonID' => $gfaktura['PersonAnsvarligID'],'DatoBokfort' => date('Y-m-d'),'Beskrivelse' => 'Faktura '.$gfaktura['FakturaID'],'Belop' => $gfaktura['FakturaSum']));
        }

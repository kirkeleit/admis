<?php
  class Prosjekter_model extends CI_Model {
    ...
    function prosjekter($filter = NULL) {
      $sql = "SELECT ProsjektID,DatoRegistrert,ProsjektAr,Prosjektnavn,FaggruppeID,(SELECT Navn FROM Faggrupper WHERE (FaggruppeID=p.FaggruppeID) LIMIT 1) AS FaggruppeNavn,PersonProsjektlederID,(SELECT Fornavn FROM Personer WHERE (PersonID=p.PersonProsjektlederID) LIMIT 1) AS PersonProsjektlederNavn,DatoProsjektstart,DatoProsjektslutt,Finansiering,Budsjettramme,(SELECT SUM(Belop) FROM Utgifter WHERE (Utgifter.ProsjektID=p.ProsjektID)) AS Utgifter,(SELECT SUM(Belop) FROM Inntekter WHERE (Inntekter.ProsjektID=p.ProsjektID)) AS Inntekter,StatusID FROM Prosjekter p WHERE 1";
      ...
      if (isset($filter['Finansiering'])) {
        $sql = $sql." AND (Finansiering=".$filter['Finansiering'].")";
      }
    ...
    function prosjekt($ID) {
      $rprosjekter = $this->db->query("SELECT ProsjektID,ProsjektAr,FaggruppeID,PersonProsjektlederID,DatoProsjektstart,DatoProsjektslutt,Prosjektnavn,Formaal,Prosjektmaal,Maalgruppe,Prosjektbeskrivelse,Arbeidstimer,Budsjettramme,Finansiering,StatusID FROM Prosjekter WHERE (ProsjektID=".$ID.") LIMIT 1");
